...
2. A workspace search for the file by name. A workspace that has relocated the standards, for
   example into `governance/standards/`, is found this way.
...
|----------|-------------------------|------|
| Agent types | `agent-types.md` | Always. Builder plane versus runtime plane |
| Operating model | `operating-model.md` | Always. Guardrail tiers, capability scope, escalation |
| Provenance | `provenance.md` | Always |
| Agent specification | `agent-specification.md` | Always |
| Agent profile schema | `agent-profile.schema.json` | Always, for validation |
| Service | `standard-service.md` | Always |
| Ontology | `ontology-schema.json` | Always, with the ontology README |

For an agent-native workspace, meaning AI-majority engineering, also load `agent-types.md`,
`principles.md`, `operating-model.md` and `provenance.md` from the agent-native standards.
...
| Runtime agents | `agent-specification.md` | `/aaa-create-runtime-agent` |
