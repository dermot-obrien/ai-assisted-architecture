...
> **Zoom relationship to C4 System Context.** A composite SBB diagram shows the **inside** of a system — its parts, ports, and connectors. The complementary **outside** view is the [C4 System Context diagram](../standard-c4-context-diagram.md): the same boundary drawn opaque, surrounded by the actors and external systems it touches. In C4 terms a composite SBB diagram **is effectively a Container / Component view (Level 2/3)**, one zoom in from the System Context view (Level 1). When a system has both, the **external systems** on the context diagram should reconcile with the **`required` boundary ports** (and external nodes) on the composite diagram — a context-level integration is, one zoom in, a required port. Author the two as a pair; the worked pair is [`example-composite/c4-context.md`](example-composite/c4-context.md) (outside) alongside [`example-composite/index.md`](example-composite/index.md) (inside).
